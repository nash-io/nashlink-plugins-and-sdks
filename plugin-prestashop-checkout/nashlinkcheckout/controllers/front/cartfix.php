...
class NashlinkCheckoutCartfixModuleFrontController extends AbstractRestController
{
    ...
    protected function processPostRequest()
    {
        ...
        elseif (isset($bpaction) && $bpaction == 'd'):
            $db_prefix = _DB_PREFIX_;
            $nashlink_table_name = '_nashlink_checkout_transactions';
            $order_table = $db_prefix . 'orders';
            $order_history_table = $db_prefix . 'order_history';

            $bp_sql = "SELECT * FROM $nashlink_table_name WHERE order_id = $orderid AND transaction_id = '$invoiceID' AND customer_key = '$customerKey'";
            $results = Db::getInstance()->executes($bp_sql);
            if (count($results) == 1):
                $this->restoreNashLinkCart($orderid);
                $this->deleteNashLinkOrder($orderid, $invoiceID, $nashlink_table_name, $order_table, $order_history_table);
                exit();
            endif;
        exit();
        endif;
    ...
    protected function restoreNashLinkCart($orderId){
    ...
    protected function deleteNashLinkOrder($orderid, $invoiceID, $nashlink_table_name, $order_table, $order_history_table)
    {
       
        #delete this order
        $bp_d = "DELETE FROM $nashlink_table_name WHERE transaction_id = '$invoiceID'";
        $db = Db::getInstance();
        $db->Execute($bp_d);
